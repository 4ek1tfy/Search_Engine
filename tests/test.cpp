...
bool operator==(const vector<Entry>& lhs, const vector<Entry>& rhs) {
    if (lhs.size() != rhs.size()) return false;
    for (size_t i = 0; i < lhs.size(); ++i) {
        if (lhs[i].doc_id != rhs[i].doc_id || lhs[i].count != rhs[i].count) {
            return false;
        }
    }
    return true;
...
TEST(TestCaseSearchServer, TestSimple) {
    ...
    const vector<vector<relative_index>> expected = {
        {
            {2, 1.0f},
            {0, 0.7f},
            {1, 0.3f}
